...
def show_page():
    ...
    st.caption("💡 Подсказка ИИ: Наведи мышь на точку или прокрути колесико, чтобы приблизить конкретную зону и увидеть вектор смещения твоей модели.")
    ...
    with col_p2:
        st.markdown("<div style='background-color:#12161A; padding:15px; border-radius:4px; border-left:4px solid #FF3344;'><h4>🎯 Выход на префайре (Wall Alignment)</h4><b>Реализовано: 12 раз</b><br>Эффективность дуэлей: 33%<br><span style='color:#FF3344;'>ИИ-Вердикт: Ошибка! Из-за eDPI 1760 ты вылетаешь слишком далеко от ребра стены. Твой прицел режет пустоту.</span></div>", unsafe_allow_html=True)
    with col_p3:
    ...
    st.success(f"🏆 **Личный рекорд побит!** На карте `{selected_map}` в этой катке ты показал самое минимальное время гашения остаточной скорости при Counter-Strafing — всего **12 тиков** до чистой остановки.")
